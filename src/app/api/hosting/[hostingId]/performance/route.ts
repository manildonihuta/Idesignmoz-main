...
import { getPerformance } from "@/services/hosting-panel.service";

export const dynamic = "force-dynamic";

const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const LIMIT = { prefix: "hosting-performance", limit: 60, windowSec: 60 };
...
export async function GET(request: NextRequest, context: Context) {
  ...
  const result = await getPerformance(ctx, hostingId);
