...
const LIMIT = { prefix: "hosting-databases", limit: 60, windowSec: 60 };
...
export async function GET(request: NextRequest, context: Context) {
  ...
  const result = await listDatabases(ctx, hostingId);
...
export async function POST(request: NextRequest, context: Context) {
  ...
  const limited = await applyRateLimit(request, { ...LIMIT, prefix: "hosting-databases-action", ip: clientIp(request) });
  if (!limited.ok) return rateLimitResponse(limited.retryAfterSec);
  ...
  switch (action) {
    case "create": {
      const result = await createDatabase(ctx, hostingId, {
    ...
    case "delete": {
      if (!validId(body.databaseId)) return Response.json({ ok: false, error: "Identificador inválido." }, { status: 400 });
      const result = await deleteDatabase(ctx, hostingId, body.databaseId);
      return Response.json(result, { status: result.ok ? 200 : result.status });
    }
    case "reset_password": {
      if (!validId(body.databaseId)) return Response.json({ ok: false, error: "Identificador inválido." }, { status: 400 });
      const result = await resetDatabasePassword(ctx, hostingId, body.databaseId);
      return Response.json(result, { status: result.ok ? 200 : result.status });
    }
    default:
