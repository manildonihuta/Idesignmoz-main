...
import { setMailboxAutoresponder } from "@/services/email.service";

export const dynamic = "force-dynamic";

const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const LIMIT = { prefix: "email-autoresponder", limit: 30, windowSec: 60 };
...
export async function PATCH(request: NextRequest, context: Context) {
  ...
  const result = await setMailboxAutoresponder(ctx, serviceId, mailboxId, body);
