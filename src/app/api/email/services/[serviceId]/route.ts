...
import { getEmailServiceDetail } from "@/services/email.service";

export const dynamic = "force-dynamic";

const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const LIMIT = { prefix: "email-service", limit: 90, windowSec: 60 };
...
export async function GET(request: NextRequest, context: Context) {
  ...
  const result = await getEmailServiceDetail(ctx, serviceId);
