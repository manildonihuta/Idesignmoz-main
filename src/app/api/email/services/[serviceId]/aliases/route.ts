...
import { createAlias } from "@/services/email.service";

export const dynamic = "force-dynamic";

const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const LIMIT = { prefix: "email-aliases-create", limit: 30, windowSec: 60 };
...
export async function POST(request: NextRequest, context: Context) {
  ...
  let body: { localPart?: unknown; destination?: unknown };
  try {
  ...
  const result = await createAlias(ctx, serviceId, body);
