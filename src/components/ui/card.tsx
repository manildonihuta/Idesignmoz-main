...
const cardVariants = cva('flex flex-col items-stretch text-card-foreground rounded-xl', {
  variants: {
    variant: {
      ...
      accent: 'bg-surface-2 shadow-xs p-1',
    },
  },
  defaultVariants: {
    variant: 'default',
...
const cardContentVariants = cva('grow p-5', {
  variants: {
    variant: {
      default: '',
      accent: 'bg-surface rounded-t-xl [&:last-child]:rounded-b-xl',
    },
  },
  defaultVariants: {
    variant: 'default',
...
const cardFooterVariants = cva('flex items-center px-5 min-h-14', {
  variants: {
    variant: {
      ...
      accent: 'bg-surface rounded-b-xl mt-[2px]',
    },
  },
  defaultVariants: {
    variant: 'default',
...
function CardTitle({ className, ...props }: React.HTMLAttributes<HTMLHeadingElement>) {
  return (
    <h3
      ...
      className={cn('text-base font-semibold leading-none tracking-tight text-paper', className)}
      {...props}
    />
  );
}
...
function CardDescription({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div data-slot="card-description" className={cn('text-sm text-muted', className)} {...props} />;
}
